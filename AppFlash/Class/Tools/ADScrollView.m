...
//创建用来缓存图片的文件夹
+ (void)initialize {
    NSString *cache = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"ADScrollView"];
    BOOL isDir = NO;
...
+ (instancetype)scrollViewWithFrame:(CGRect)frame imageArray:(NSArray *)imageArray {
...
- (void)imageClick {
    ...
    } else if ([_delegate respondsToSelector:@selector(scrollView:didClickImage:)]){
        [_delegate scrollView:self didClickImage:self.currIndex];
    }
}
...
- (void)downloadImages:(int)index {
    ...
    //从沙盒中取图片
    NSString *path = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"ADScrollView"] stringByAppendingPathComponent:[key lastPathComponent]];
